                                    ...
                                    LPLC_STRINGS lpOutStr)
{
    ...
    iCodePage = ProcessCodePage(lpInStr->szCodePage, _psetloc_data);
    ...
    if (lpOutStr)
    {
        ...
        else if ((*pfnGetLocaleInfoA)(_psetloc_data->lcidLanguage, LOCALE_SENGLANGUAGE,
                                 lpOutStr->szLanguage, MAX_LANG_LEN) == 0)
            return FALSE;
        if ((*pfnGetLocaleInfoA)(_psetloc_data->lcidCountry, LOCALE_SENGCOUNTRY,
...
static BOOL CALLBACK LangCountryEnumProc (LPSTR lpLcidString)
{
                      ...
                      (__LCID_LANGUAGE | __LCID_EXISTS))
    {
        ...
        if ((*pfnGetLocaleInfoA)(lcid, _psetloc_data->bAbbrevLanguage ? LOCALE_SABBREVLANGNAME
                                                       : LOCALE_SENGLANGUAGE,
                           rgcInfo, sizeof(rgcInfo)) == 0)
        {
            //  set error condition and exit
            ...
            else
            {
                ...
                if (!_psetloc_data->lcidLanguage)
                    _psetloc_data->lcidLanguage = lcid;
                               ...
                               && !_strnicmp(_psetloc_data->pchLanguage, rgcInfo, _psetloc_data->iPrimaryLen))
        {
...
static BOOL CALLBACK CountryEnumProc (LPSTR lpLcidString)
{
    ...
    if ((*pfnGetLocaleInfoA)(lcid, _psetloc_data->bAbbrevCountry ? LOCALE_SABBREVCTRYNAME
                                                  : LOCALE_SENGCOUNTRY,
                       rgcInfo, sizeof(rgcInfo)) == 0)
    {
        //  set error condition and exit
...
static int ProcessCodePage (LPSTR lpCodePageStr, _psetloc_struct _psetloc_data)
{
    ...
    if (!lpCodePageStr || !*lpCodePageStr || !strcmp(lpCodePageStr, "ACP"))
    {
        ...
        if ((*pfnGetLocaleInfoA)(_psetloc_data->lcidCountry, LOCALE_IDEFAULTANSICODEPAGE,
    ...
    else if (!strcmp(lpCodePageStr, "OCP"))
    {
        ...
        if ((*pfnGetLocaleInfoA)(_psetloc_data->lcidCountry, LOCALE_IDEFAULTCODEPAGE,
                                 chCodePage, sizeof(chCodePage)) == 0)
            return 0;
        lpCodePageStr = chCodePage;
    }
