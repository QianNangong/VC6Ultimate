...
void CDatabase::ExecuteSQL(LPCTSTR lpszSQL)
{
	...
	TRY
	{
		...
		UCHAR* lpszTemp = (UCHAR*)T2A((LPTSTR)lpszSQL);
	...
	SQLULEN* pdwRowsFetched)
{
	...
	{
		SQLULEN uRowsFetched;
		AFX_ODBC_CALL(::SQLExtendedFetch(m_hstmt, wFetchType,
			nRow, &uRowsFetched, m_rgRowStatus));
		*pdwRowsFetched = (DWORD)uRowsFetched;
...
void CRecordset::BuildUpdateSQL()
{
	...
	if(m_dwOptions & firstBulkAdd)
	{
		...
		USES_CONVERSION;
		UCHAR* lpszSQL = (UCHAR*)T2A((LPTSTR)(LPCTSTR)m_strUpdateSQL);
