        ...
        )
{
        ...
        for (i = 0; i < min((int)pHead->nDataSize, MAXPRINT); i++)
        {
            ch = pbData(pHead)[i];
#ifdef _MT
            printbuff[i] = __isprint_mt(ptloci, ch) ? ch : ' ';
#else  /* _MT */
...
#ifdef _MT
        /* Acquire thread locale before taking _HEAP_LOCK, to avoid potential
         * deadlock with _SETLOCALE_LOCK
         */
...
#ifdef _MT
                        _printMemBlockData(ptloci, pHead);
#else  /* _MT */
