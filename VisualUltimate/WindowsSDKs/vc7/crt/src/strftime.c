        ...
        )
{
        ...
        /* Ensure that there is some space for output */
        if(maxsize==0)
        {
            _ASSERTE(("Zero length output buffer passed to strftime",0));
            return 0;
        }
        ...
        while (left > 0)
        {
...
#ifdef _MT
                if(!_expandtime (ptloci, *format, timeptr, &string,
#else  /* _MT */
            ...
            default:
                ...
                if(
#ifdef _MT
                    __isleadbyte_mt(ptloci, (int)(*format))
#else  /* _MT */
                    isleadbyte((int)(*format))
#endif  /* _MT */
                    && left > 1)
                {
        ...
        else
        {
            if(left==0)
            {
                /* step back 1 char so we have space to terminate */
                string--;
            }

            /* ensure whatever we return is null terminated */
            *(string)='\0';
