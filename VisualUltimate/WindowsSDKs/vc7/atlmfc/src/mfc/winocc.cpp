...
public:
	...
	{
		ASSERT(m_pList == iterator.m_pList);
...
static POSITION FindSiteOrWnd(const COleControlContainer *pCtrlCont, const COleControlSiteOrWnd *pSiteOrWnd)
{
	if (!pCtrlCont)
	{
		return NULL;
	}
	CDlgControlIterator iterCtl = *pCtrlCont;
	while (!iterCtl.IsEnd())
	{
...
static POSITION FindSiteOrWndWithFocus(const COleControlContainer *pCtrlCont)
{
	if (!pCtrlCont)
	{
		return NULL;
	...
	while (!iterCtl.IsEnd())
	{
		...
		if ((hwndControl && hwndControl == ::GetFocus()) ||
			(pSiteOrWnd->m_pSite && pSiteOrWnd->m_pSite == pCtrlCont->m_pSiteFocus))
		{
			return iterCtl.GetPosition();
		}
		iterCtl.MoveNext();
...
COleControlSiteOrWnd* CWnd::GetPrevDlgGroupItem(COleControlSiteOrWnd *pCurSiteOrWnd) const
{
	...
	POSITION pos = pCurSiteOrWnd ? FindSiteOrWnd(m_pCtrlCont, pCurSiteOrWnd) : FindSiteOrWndWithFocus(m_pCtrlCont);
	if (!pos)
	{
		return NULL;
	}
