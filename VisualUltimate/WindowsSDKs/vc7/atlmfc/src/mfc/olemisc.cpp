	...
	LPSIZEL         lpSizelHim)
{
	...
	if (fIsLink && lpszFullUserTypeName != NULL)
	{
		...
		VERIFY(AfxLoadString(AFX_IDS_PASTELINKEDTYPE, szLinkedTypeFmt, _countof(szLinkedTypeFmt)));
...
DVTARGETDEVICE* AFXAPI _AfxOleCreateTargetDevice(LPDEVNAMES pDN, LPDEVMODE pDM)
{
	...
	int nDriverNameSize = (int)((lpszDriverNameOle == NULL) ? 0 : (ocslen(lpszDriverNameOle)+1)*sizeof(OLECHAR));
	int nDeviceNameSize = (int)((lpszDeviceNameOle == NULL) ? 0 : (ocslen(lpszDeviceNameOle)+1)*sizeof(OLECHAR));
	int nPortNameSize = (int)((lpszPortNameOle == NULL) ? 0 : (ocslen(lpszPortNameOle)+1)*sizeof(OLECHAR));
