...
BOOL AFXAPI AfxRegisterClass(WNDCLASS* lpWndClass)
{
	...
	if (afxContextIsDLL)
	{
		...
		TRY
		{
			...
			if (lstrlen(lpszUnregisterList) + 1 +
				lstrlen(lpWndClass->lpszClassName) + 1 <
				_countof(pModuleState->m_szUnregisterList))
			{
			...
			{
				bRet = FALSE;
				::UnregisterClass(lpWndClass->lpszClassName, lpWndClass->hInstance);
			}
		}
		CATCH_ALL(e)
		{
	...
	HCURSOR hCursor, HBRUSH hbrBackground, HICON hIcon)
{
	...
	if (hCursor == NULL && hbrBackground == NULL && hIcon == NULL)
		_stprintf(lpszName, _T("Afx:%p:%x"), hInst, nClassStyle);
	else
		_stprintf(lpszName, _T("Afx:%p:%x:%p:%p:%p"), hInst, nClassStyle,
