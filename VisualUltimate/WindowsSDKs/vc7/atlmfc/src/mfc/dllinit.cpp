...
HINSTANCE AFXAPI AfxFindResourceHandle(LPCTSTR lpszName, LPCTSTR lpszType)
{
		...
		pDLL = pDLL->m_pNextDLL)
	{
		...
		{
			//Cache up the hInst for later use
			hInstCache.Add(pDLL->m_hResource);
		}
	}
	AfxUnlockGlobals(CRIT_DYNLINKLIST);

	//Checking the hInst outside CRIT_DYNLINKLIST because it requires a loader lock which 
	//can potentially cause a deadlock when we are loading/unloading modules.
