						...
						DWORD_PTR dwCategory, UINT nLevel, const CHAR *pszFormat, va_list ptr)
{
	...
	if(ShouldTraceOutput(dwModule, dwCategory, nLevel, &pCategory, &pfnCrtDbgReport))
	{
		...
		{
			int nTemp = _snprintf(szBuf + nLen, nCount - nLen, "%s(%d) : ", pszFileName, nLine);
			if( nTemp < 0 )
				nLen = nCount;
			else
				nLen += nTemp;
		...
		{
			int nTemp = _snprintf(szBuf + nLen, nCount - nLen, "%S: ", pCategory->Name());
			if( nTemp < 0 )
				nLen = nCount;
			else
				nLen += nTemp;
						...
						DWORD_PTR dwCategory, UINT nLevel, const WCHAR *pszFormat, va_list ptr)
{
	...
	if(ShouldTraceOutput(dwModule, dwCategory, nLevel, &pCategory, &pfnCrtDbgReport))
	{
		if(g_Allocator.GetProcess()->m_bFileNameAndLineNo)
		{
			int nTemp = _snwprintf(szBuf + nLen, nCount - nLen, L"%S(%d) : ", pszFileName, nLine);
			if( nTemp < 0 )
				nLen = nCount;
			else
				nLen += nTemp;
		...
		if(pCategory && g_Allocator.GetProcess()->m_bFuncAndCategoryNames)
		{
			int nTemp = _snwprintf(szBuf + nLen, nCount - nLen, L"%s: ", pCategory->Name());
			if( nTemp < 0 )
				nLen = nCount;
			else
				nLen += nTemp;
...
DWORD_PTR __stdcall AtlTraceOpenProcess(DWORD idProcess)
{
	...
	char szBuf[64];
	_snprintf(szBuf, sizeof(szBuf), g_pszKernelObjFmt, g_pszAllocFileMapName, idProcess);
...
BOOL __stdcall AtlTraceGetProcessInfo(DWORD_PTR dwProcess, ATLTRACEPROCESSINFO* pProcessInfo)
{
	...
	if(pProcess)
	{
		lstrcpynW(pProcessInfo->szName, pProcess->Name(), sizeof(pProcessInfo->szName)/sizeof(pProcessInfo->szName[0]));
...
void __stdcall AtlTraceGetModuleInfo(DWORD_PTR dwProcess, int iModule, ATLTRACEMODULEINFO* pModuleInfo)
{
	...
	lstrcpynW(pModuleInfo->szName, pModule->Name(), sizeof(pModuleInfo->szName)/sizeof(pModuleInfo->szName[0]));
	lstrcpynW(pModuleInfo->szPath, pModule->Path(), sizeof(pModuleInfo->szPath)/sizeof(pModuleInfo->szPath[0]));
...
void __stdcall AtlTraceGetCategoryInfo(DWORD_PTR dwProcess, DWORD_PTR dwModule, int iCategory, ATLTRACECATEGORYINFO* pCategoryInfo)
{
	...
	lstrcpynW(pCategoryInfo->szName, pCategory->Name(), sizeof(pCategoryInfo->szName)/sizeof(pCategoryInfo->szName[0]));
