...
{
	if (!::SystemTimeToVariantTime(const_cast<SYSTEMTIME *>(&systimeSrc), &m_dt))
	{
		m_status = invalid;
		return *this;
	}

	// Since SystemTimeToVariantTime tries to fix invalid dates, we cannot rely on it to tell us
	// whether the original SYSTEMTIME was incorrect. We check that using an additional call
	// to VariantTimeToSystemTime and then compare all SYSTEMTIME fields except for wDayOfWeek and
	// wMilliseconds.
	SYSTEMTIME sysTime;
	::ZeroMemory(&sysTime, sizeof(SYSTEMTIME));

	::VariantTimeToSystemTime(m_dt, &sysTime);
	m_status = (systimeSrc.wYear == sysTime.wYear &&
				...
				systimeSrc.wSecond == sysTime.wSecond) ? valid : invalid;
	...
	int nHour, int nMin, int nSec) throw()
{
	...
	if (!::SystemTimeToVariantTime(&st, &m_dt))
