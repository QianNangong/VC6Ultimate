...
inline BOOL AtlMimeCharsetFromCodePage(LPSTR szCharset, UINT uiCodePage, IMultiLanguage* pMultiLanguage, size_t cch) throw()
{
	...
	if (!pMultiLanguage)
	{
		...
		{
			ATLASSERT(sizeof(ATLSMTP_DEFAULT_CSET) / sizeof(ATLSMTP_DEFAULT_CSET[0]) <= cch);
