...
inline size_t SetRfc822Time(LPSTR szDate, size_t dwLen) throw()
{
		...
		*spBuf = '\0';
		...
		if (bRet && dwHeaderPartLength != 0)
		{
			...
			if(sizeof(szFrom)/sizeof(szFrom[0])-1 > ATLSMTP_MAX_LINE_LENGTH -2 -dwOffset )
				return FALSE;
			memcpy(pSendBuffer+dwOffset, szFrom, sizeof(szFrom)/sizeof(szFrom[0])-1);
		...
		if (bRet && m_strSubject.GetLength() > 0)
		{
			...
			if (bRet && dwLength != 0)
			{
				...
				if(sizeof(szSubject)/sizeof(szSubject[0])-1 > ATLSMTP_MAX_LINE_LENGTH -2 -dwOffset )
					return FALSE;
				memcpy(pSendBuffer+dwOffset, szSubject, sizeof(szSubject)/sizeof(szSubject[0])-1);
		...
		if (bRet && m_strTo.GetLength() > 0)
		{
			...
			if(sizeof(szTo)/sizeof(szTo[0])-1 > ATLSMTP_MAX_LINE_LENGTH -2 -dwOffset )
					return FALSE;
			memcpy(pSendBuffer+dwOffset, szTo, sizeof(szTo)/sizeof(szTo[0]) -1);
		...
		if (bRet && m_strCc.GetLength() > 0)
		{
			...
			if(sizeof(szCC)/sizeof(szCC[0])-1 > ATLSMTP_MAX_LINE_LENGTH -2 -dwOffset )
				return FALSE;
			memcpy(pSendBuffer+dwOffset, szCC, sizeof(szCC)/sizeof(szCC[0]) -1);
		...
		_ATLTRY
		{
			CT2CA szDisplayNameA(szDisplayName);
		 	if (szDisplayNameA == NULL || strlen(szDisplayNameA) > MAX_PATH)
		 		return FALSE;
			strcpy(m_szDisplayName, szDisplayNameA);
			return TRUE;
		}
		_ATLCATCHALL()
		{
...
protected:
	...
	inline BOOL MakeBoundary(LPSTR szBoundary, int nBufLen) throw()
	{
		ATLASSERT(szBoundary != NULL);
