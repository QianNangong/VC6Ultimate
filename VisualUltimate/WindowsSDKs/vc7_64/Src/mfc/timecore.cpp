...
//      %% - percent sign
{
	...
	// Code to ensure that a big enough buffer
	// is allocated at the beginning.
	szBuffer.GetBuffer(maxTimeBufferSize);
	...
	while ((ch = *pFormat++) != '\0')
	{
		...
		if (ch == '%')
		{
			...
			case '%':
				...
				break;
			case 'D':
				buf.Format(_T("%ld"), GetDays());
				break;
				
			case 'H':
				buf.Format(_T("%02d"), GetHours());
				break;
				
			case 'M':
				buf.Format(_T("%02d"), GetMinutes());
				break;
			case 'S':
				buf.Format(_T("%02d"), GetSeconds());
				break;
			}
