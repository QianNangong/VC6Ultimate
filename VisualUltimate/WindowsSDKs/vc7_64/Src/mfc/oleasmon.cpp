...
private:
		...
		LPCOLESTR szStatusText)
	{
		...
		TRY
		{
			LPCTSTR szStatusText2 = OLE2CT_EX(szStatusText, _ATL_SAFE_ALLOCA_DEF_THRESHOLD);
			if (NULL == szStatusText2)
			{
				return E_OUTOFMEMORY;
			}
			m_pOwner->OnProgress(ulProgress, ulProgressMax, ulStatusCode, szStatusText2);
		}
		CATCH_ALL(e)
		{
			HRESULT hr = ResultFromScode(COleException::Process(e));
	...
	STDMETHOD(OnStopBinding)(HRESULT hresult, LPCOLESTR szError)
	{
		...
		TRY
		{
			...
			LPCTSTR szError2 = OLE2CT_EX(szError, _ATL_SAFE_ALLOCA_DEF_THRESHOLD);
			if (NULL == szError2)
			{
				return E_OUTOFMEMORY;
			}
			m_pOwner->OnStopBinding(hresult, szError2);
