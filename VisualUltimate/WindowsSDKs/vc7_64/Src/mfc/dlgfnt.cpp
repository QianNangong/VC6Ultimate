...
DWORD CFontDialog::FillInLogFont(const CHARFORMAT& cf)
{
	...
	if (cf.dwMask & CFM_FACE)
	{
		...
		lstrcpyn(m_lf.lfFaceName, A2T((LPSTR)cf.szFaceName), _countof(m_lf.lfFaceName));
	}
	else
	{
...
void CFontDialog::GetCharFormat(CHARFORMAT& cf) const
{
	...
	if ((m_cf.Flags & CF_NOFACESEL) == 0)
	{
		...
		lstrcpynA(cf.szFaceName, T2A((LPTSTR)(LPCTSTR)GetFaceName()), _countof(cf.szFaceName));
	}
