	...
	if (rgCmds != NULL)
	{
		...
		else
		{
				...
				(pcmdtext->cmdtextf != OLECMDTEXTF_NONE))
			{
				...
				pcmdtext->cwActual = min((ULONG)state.m_strText.GetLength(),pcmdtext->cwBuf - 1);

#ifdef _UNICODE
				lstrcpynW(pcmdtext->rgwz, (LPCTSTR) state.m_strText,pcmdtext->cwBuf);
#elif defined(OLE2ANSI)
				lstrcpyn(pcmdtext->rgwz, state.m_strText,pcmdtext->cwBuf);
#else
				lstrcpynW(pcmdtext->rgwz, T2W((LPCTSTR) state.m_strText),pcmdtext->cwBuf);
#endif
			}
		}
	}
